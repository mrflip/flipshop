...
import       { SizeOverrides }                    from './GearwrenchOverrides.ts'
//
export      *                                     from './GearwrenchOverrides.ts'
...
const DriverKindToPrefix = {
  ...
  intsq:           'Dr',   torx:            'T',   torxtp:          'TP',   triple_square:   'Tsq',
...
export function parseProductPage(filepath: TY.Anypath, textblob: string): GearwrenchSocketT {
  ...
  if (result.drive_kind === 'inthex')   { result.sizing = result.sizing.replace(/^(H)?/,    'H') }
  ...
  const driver_sz =  (driverSizingPrefix + result.sizing.replace(/^([THE]+|P[hz]|Sl)/, '')) as FE.ToolDrive
  result.targets = DriveToDriven[driver_sz] ?? {}
  if (/socket_(exthex|extstar|bit)$/.test(result.socket_kind)) {
    if (! Fastener.FastenerEnums.DriverSizingVals.includes(driver_sz)) { console.warn('DriverSizingVals lacks:', driver_sz, result) }
