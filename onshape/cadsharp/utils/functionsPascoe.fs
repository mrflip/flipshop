...
export predicate QFinderPredicate(definition is map, suffix is string, displaySuffix is string)
{
    ...
    {
        annotation { "Group Name" : "Query finder" ~ displaySuffix, "Collapsed By Default" : false }
        {
            if (definition["queryFinder" ~ suffix])
            {
                ...
                if (definition["searchType" ~ suffix] != SearchType.EVERYTHING)
                {
                    ...
                    {
                        annotation { "Name" : "(A) Property type", "UIHint" : UIHint.FOCUS_ON_VISIBLE }
                        definition["propertyType" ~ suffix] is Property;

                        if (definition["propertyType" ~ suffix] == Property.CUSTOM)
                        {
                ...
                annotation { "Name" : "Filter entities", "Default" : false, "UIHint" : [UIHint.ALWAYS_HIDDEN, UIHint.UNCONFIGURABLE] }
                definition["showFilterEntities" ~ suffix] is boolean;

                if (definition["showFilterEntities" ~ suffix])
                {
                    annotation { "Name" : "Filter entities", "Default" : false }
                    definition["filterEntities" ~ suffix] is boolean;
                }

                if (definition["filterEntities" ~ suffix])
                {
                    ...
                    annotation { "Name" : "Vertices", "Default" : false, "UIHint" : [UIHint.ALWAYS_HIDDEN, UIHint.UNCONFIGURABLE] }
                    definition["showVertices" ~ suffix] is boolean;

                    if (definition["showVertices" ~ suffix])
                    {
                        annotation { "Name" : "Vertices", "Default" : false }
                        definition["vertices" ~ suffix] is boolean;
                    }
...
export function QFinderFunction(context is Context, definition is map, suffix is string) returns Query
{
    ...
    if (definition["queryFinder" ~ suffix])
    {
        if (definition["searchType" ~ suffix] != SearchType.EVERYTHING)
        {
            if (definition["searchType" ~ suffix] == SearchType.ATTRIBUTE)
            {
                if (definition["attribute" ~ suffix] is array)
                {
                    for (var s in definition["attribute" ~ suffix])
                    {
                        searchQuery = qUnion([searchQuery, qHasAttribute(s)]);
                    }
                }
                else
                {
            ...
            {
                if (definition["identity" ~ suffix] is array)
                {
                    for (var s in definition["identity" ~ suffix])
                    {
                        searchQuery = qUnion([searchQuery, qNamed(s)]);
                    }
                }
                else
                {
            ...
            {
                if (definition["transientId" ~ suffix] is array)
                {
                    for (var s in definition["transientId" ~ suffix])
                    {
                        const transientQuery = s as Query;
        ...
        if (definition["filterEntities" ~ suffix])
        {
            ...
            if (definition["vertices" ~ suffix])
            {
                everything = qUnion([everything, qEntityFilter(searchQuery, EntityType.VERTEX)]);
            }
 ...
 *      @field showFilterEntities {boolean} : Filter the found entities. Default `true`. @eg `true`
 ...
 *      @field showVertices {boolean} : Whether vertex inputs are visible. Default `false`. @eg `false`
 ...
 *      @field filterEntities {boolean} : Default value for filter entities. Default `false`. @eg `false`
 ...
 *      @field vertices {boolean} : Default value for vertex. Default `false`. @eg `false`
...
export function QFinderSetDefaultsAndVisibility(context is Context, definition is map, oldDefinition is map, qFinderVisibleInputs is map, qFinderDefaultInputs, suffix is string) returns map
{
    if (!definition["defaultsHaveBeenSet" ~ suffix] || definition["queryFinder" ~ suffix] != oldDefinition["queryFinder" ~ suffix])
    {
        ...
        definition["showFilterEntities" ~ suffix] = qFinderVisibleInputs.showFilterEntities;
        ...
        definition["showVertices" ~ suffix] = qFinderVisibleInputs.showVertices;
        ...
        definition["filterEntities" ~ suffix] = qFinderDefaultInputs.filterEntities;
        ...
        definition["vertices" ~ suffix] = qFinderDefaultInputs.vertices;
    ...
    if (definition["searchType" ~ suffix] == SearchType.PROPERTY && (!isUndefinedOrEmptyString(definition["propertyID" ~ suffix]) || definition["propertyType" ~ suffix] != Property.CUSTOM))
    {
        ...
        for (var i = 0; i < qty; i += 1)
        {
            try silent
            {
                ...
                var propDef = {};
                propDef.entity = entity;
                propDef.propertyType = PropertyType[definition["propertyType" ~ suffix]->toString()];

                if (definition["propertyType" ~ suffix] == Property.CUSTOM)
                    propDef.customPropertyId = definition["propertyID" ~ suffix];

                if (propDef.customPropertyId is array)
                {
                    for (var s in propDef.customPropertyId)
                    {
                        const propValue = getProperty(context, s);
                ...
                {
                    const propValue = getProperty(context, propDef);
