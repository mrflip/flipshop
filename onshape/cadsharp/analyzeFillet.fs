//Copied and modified from Onshape standard source
...
export import(path : "onshape/std/geometry.fs", version : "1777.0");
icon::import(path : "14c94cfce8179a64cbb0dfcb", version : "d56f66357d23f89056d83732");
descriptionImage::import(path : "11d17ecb740cd287c18a2ca4", version : "eef4f5222d9b56fce99b8d4e");
 ...
 */
annotation {
        "Feature Type Name" : "Analyze fillet",
        "Manipulator Change Function" : "manipulatorChange",
        "Filter Selector" : "allparts",
        "Editing Logic Function" : "cadsharpUrlEditLogic",
        "Icon" : icon::BLOB_DATA,
        "Feature Type Description" : "<b> Summary </b> <br> See where the fillet will fail by splitting the part into segments before the fillet takes place. <br>",
        "Description Image" : descriptionImage::BLOB_DATA }
export const analyzeFillet = defineFeature(function(context is Context, id is Id, definition is map)
    precondition
    {
        annotation { "Name" : "Analyze fillet", "Default" : true }
        definition.analyzeFillet is boolean;

        annotation { "Name" : "offset 1", "UIHint" : UIHint.ALWAYS_HIDDEN }
        isLength(definition.offset1, { (inch) : [0, 0, 10000] } as LengthBoundSpec);

        annotation { "Name" : "offset 2", "UIHint" : UIHint.ALWAYS_HIDDEN }
        isLength(definition.offset2, { (inch) : [0, 0, 10000] } as LengthBoundSpec);

        annotation { "Name" : "Section count" }
        isInteger(definition.sectionCount, { (unitless) : [0, 5, 100] } as IntegerBoundSpec);

        annotation { "Name" : "Split direction", "Filter" : EntityType.FACE || BodyType.MATE_CONNECTOR, "MaxNumberOfPicks" : 1 }
        definition.splitDirection is Query;

        annotation { "Name" : "Debug color", "Default" : true }
        definition.debugColor is boolean;

        annotation { "Name" : "Fillet type", "UIHint" : UIHint.ALWAYS_HIDDEN }
    ...
    }
    {
        ...
        if (size(evaluateQuery(context, qEntityFilter(definition.splitDirection, EntityType.FACE))) > 0)
        {
            localPlane = evFaceTangentPlane(context, {
                        "face" : definition.splitDirection,
                        "parameter" : vector(0.5, 0.5)
                    });

            localCsys = coordSystem(localPlane);
        }
        else
        {
            localCsys = evMateConnector(context, {
                        "mateConnector" : definition.splitDirection
                    });

            localPlane = plane(localCsys);
        }

        const ownerBody = makeRobustQuery(context, qOwnerBody(definition.entities));
        ...
        // const x = bbox.maxCorner[0] - bbox.minCorner[0];
        // const y = bbox.maxCorner[1] - bbox.minCorner[1];
        var z = bbox.maxCorner[2] - bbox.minCorner[2];

        const z2 = z;

        // Adjust z by manipulator 2 offset
        z = z - definition.offset2 - definition.offset1;

        const step = z / definition.sectionCount;

        // Reverse direction if needed
        if ((worldToPlane3D(localPlane) * bbox.maxCorner)[2] < 0)
        {
            localPlane = plane(localPlane.origin, -localPlane.normal, localPlane.x);
        }

        const manipPlane = localPlane;

        // Adjust local plane with the manip offset
        localPlane = plane(planeToWorld3D(localPlane) * vector(0 * inch, 0 * inch, definition.offset1), localPlane.normal, localPlane.x);

        // Manip plane 2
        const manipPlane2 = plane(planeToWorld3D(manipPlane) * vector(0 * inch, 0 * inch, z2), -localPlane.normal, localPlane.x);

        var manipDef = {};

        manipDef.linearManip1 = linearManipulator({
                    "base" : manipPlane.origin,
                    "direction" : manipPlane.normal,
                    "offset" : definition.offset1
                });

        manipDef.linearManip2 = linearManipulator({
                    "base" : manipPlane2.origin,
                    "direction" : manipPlane2.normal,
                    "offset" : definition.offset2
                });

        addManipulators(context, id, manipDef);


        if (definition.analyzeFillet)
        {
            var toSplit = ownerBody;
            var toDelete = qNothing();

            for (var i = 0; i < definition.sectionCount; i += 1)
            {
                if (i > 0)
                {
                    const tempPlane = plane(planeToWorld3D(localPlane) * vector(0 * inch, 0 * inch, step * i), localPlane.normal, localPlane.x);

                    opPlane(context, id + i + "plane1", {
                                "plane" : tempPlane
                            });

                    const splitPlane = qCreatedBy(id + i + "plane1", EntityType.FACE);
                    toDelete = qUnion([toDelete, splitPlane]);

                    try
                    {
                        opSplitPart(context, id + i + "splitPart1", {
                                    "targets" : toSplit,
                                    "tool" : splitPlane
                                });

                        toSplit = qSplitBy(id + i + "splitPart1", EntityType.BODY, false);
                    }
                    catch
                    {
            ...
            const evEdges = evaluateQuery(context, entities);

            for (var i = 0; i < size(evEdges); i += 1)
            {
                definition.entities = evEdges[i];
...
function performEdgeFillet(context is Context, topLevelId is Id, definition is map)
{
    try
    {
        ...
        {
            debug(context, "TRUE", DebugColor.RED);
    ...
    {
        debug(context, qOwnerBody(definition.entities), DebugColor.RED);

        if (definition.debugColor)
        {
            setProperty(context, {
                        "entities" : qOwnerBody(definition.entities),
                        "propertyType" : PropertyType.APPEARANCE,
                        "value" : color(221 / 255, 82 / 255, 40 / 255, 1)
                    });
        }
    }
}
...
{
    // if (newManipulators["linearManip0"] is map)
    // {
    //     var newDepth is ValueWithUnits = newManipulators["linearManip0"].offset;
    //     definition.distance = newDepth;
    // }

    if (newManipulators["linearManip1"] is map)
    {
        var newDepth is ValueWithUnits = newManipulators["linearManip1"].offset;
        definition.offset1 = newDepth;
    }

    if (newManipulators["linearManip2"] is map)
    {
        var newDepth is ValueWithUnits = newManipulators["linearManip2"].offset;
        definition.offset2 = newDepth;
    }
